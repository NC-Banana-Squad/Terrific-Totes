...
def sales_order(df):
    ...
    df['created_at'] = df['created_at'].apply( lambda x: x if '.' in x else x + ".000000" ) 
    df['created_at'] = pd.to_datetime(df['created_at'], format="%Y-%m-%d %H:%M:%S.%f", errors='coerce')
    df['created_date'] = df['created_at'].dt.date
    df['created_time'] = df['created_at'].dt.time

    df['last_updated'] = df['last_updated'].apply( lambda x: x if '.' in x else x + ".000000" ) 
    df['last_updated'] = pd.to_datetime(df['last_updated'], format="%Y-%m-%d %H:%M:%S.%f", errors='coerce')
